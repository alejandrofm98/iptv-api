...
class CircuitBreakerService:
    ...
    def get_all_status(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            hostname: {"state": m.state.value, "failures": m.failures, "successes": m.successes}
        ...
        for attempt in range(self.config.max_attempts):
            try:
                ...
                if attempt < self.config.max_attempts - 1:
                    ...
                    logger.warning(
                        f"🔄 Retry {attempt + 1}/{self.config.max_attempts}: {e}. Waiting {delay:.1f}s"
                    )
...
class StreamBuffer:
    ...
    def __init__(self, config: StreamBufferConfig | None = None):
    ...
    async def should_start_streaming(self) -> bool:
        ...
        if min_chunks_met and time_met:
            self._streaming_started = True
            logger.info(
                f"▶️ Buffer ready: {self._buffered_chunks} chunks ({self._buffered_bytes / 1024:.1f}KB)"
            )
            return True
...
class ResilienceService:
    ...
    def __init__(
        ...
        buffer_config: StreamBufferConfig | None = None,
    ):
    ...
    async def execute_with_resilience(self, url: str, func: Callable[..., T], *args, **kwargs) -> T:
        """
